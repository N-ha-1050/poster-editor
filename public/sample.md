...
著者をマップ形式で指定します。キーが著者名の文字列、値が所属の文字列かその配列、または、 `null` になります。 `null` や空文字列の場合は所属なしとして扱われます。同一の所属の著者が複数いる場合は、所属を同じ値にすることでまとめて表示されます。
...
画面左のエディターには、マークダウン形式でテキストを入力できます。
